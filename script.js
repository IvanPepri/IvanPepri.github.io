...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 1. CÁLCULO DINÁMICO DE LA EDAD ---
    
    // ¡¡IMPORTANTE!! Reemplaza esto con tu fecha de nacimiento
    const FECHA_NACIMIENTO = '2000-07-15'; // Formato AÑO-MES-DÍA
    ...
    if (spanEdad) {
        try {
            ...
            if (mes < 0 || (mes === 0 && hoy.getDate() < cumple.getDate())) {
                edad--; // Resta 1 si aún no ha sido tu cumpleaños este año
            }
            
            spanEdad.textContent = edad; // Escribe la edad en el HTML

        } catch (error) {
            console.error("Error al calcular la edad. Verifica la fecha de nacimiento.", error);
            spanEdad.textContent = "(Fecha de nacimiento no válida)";
        }
    }
